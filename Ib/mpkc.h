...
static inline void
to_maddusb_format_mq(uint8_t* z, const uint8_t* x, unsigned _n, unsigned m) {
	assert(m <= 128);
...
static inline void
maddusb_to_normal_mq(uint8_t* z, const uint8_t* x, unsigned _n, unsigned m) {
	assert(m <= 128);
	unsigned n = (_n + 1) * (_n + 2) / 2;
	uint8_t temp[256];
	while (n > 1) {
...
static inline void _generate_quadratic_terms_revlex_gf31_n(
	uint8_t* r, const uint8_t* a, unsigned n) {
...
static inline void
_mpkc_pub_map_gf31(uint8_t* z, const uint8_t* pk_mat, const uint8_t* w) {
	assert(0 == (_PUB_N % 2));
	...
	gf31mat_prod(
		tmp, mat_quadratic, _PUB_M_BYTE, TERMS_QUAD_POLY(_PUB_N) - _PUB_N - 1,
		quad_terms);
	gf31v_add(r, tmp, _PUB_M_BYTE);

	const uint8_t* vec_constant =
		pk_mat + (TERMS_QUAD_POLY(_PUB_N) - 1) * (_PUB_M_BYTE);
...
static inline void
mpkc_pub_map_gf31_ref(uint8_t* z, const uint8_t* pk_mat, const uint8_t* w) {
	_mpkc_pub_map_gf31(z, pk_mat, w);
}

#ifdef _TWO_COL_MAT_
static inline void _mpkc_pub_map_gf31_n_m(
	uint8_t* z, const uint8_t* pk_mat, const uint8_t* w, unsigned n,
	unsigned m) {
	assert(0 == (n % 2));
	assert(128 >= m);
	assert(140 >= n);
...
static inline void mpkc_pub_map_gf31_n_m_ref(
	uint8_t* z, const uint8_t* pk_mat, const uint8_t* w, unsigned n,
	unsigned m) {
	_mpkc_pub_map_gf31_n_m(z, pk_mat, w, n, m);
}

static inline void mpkc_interpolate_gf31(
	uint8_t* poly, void (*quad_poly)(void*, const void*, const void*),
	const void* key) {
	...
	for (unsigned i = 0; i < n_var; i++) {
		...
		gf31v_mul_scalar(
			tmp_r0, 16, _PUB_M_BYTE);			 /// 2*v * 16 = 32*v mod 31 = v
		gf31v_add(tmp_r1, tmp_r0, _PUB_M_BYTE);  ///  v^2
		memcpy(poly + _PUB_M_BYTE * i, tmp_r0, _PUB_M_BYTE);
		memcpy(
			poly + _PUB_M_BYTE * (n_var + IDX_QTERMS_REVLEX(i, i)), tmp_r1,
			_PUB_M_BYTE);
	}
	...
	for (unsigned i = 0; i < n_var; i++) {
		...
		memcpy(
			tmp_r2, poly + _PUB_M_BYTE * (n_var + IDX_QTERMS_REVLEX(i, i)),
			_PUB_M_BYTE);
		...
		for (unsigned j = 0; j < i; j++) {
			...
			gf31v_sub(
				tmp_r0, poly + _PUB_M_BYTE * (n_var + IDX_QTERMS_REVLEX(j, j)),
				_PUB_M_BYTE);
