...
static inline void openssl_prng_bytes(unsigned char* a, unsigned _num_byte) {
	if (0 == fips_mode_on) {
...
void prng_bytes(unsigned char* a, unsigned _num_byte) {
	if (1 == user_source)
		userrand_bytes(a, _num_byte);
	else if (2 == user_source)
		openssl_prng_bytes(a, _num_byte);
	else {
		for (unsigned i = 0; i < _num_byte; i++) {
			if (0 == prng_state) {
				randombytes(_prng_buffer, _PRNG_BUFFER_SIZE);
			}
	...
	if (prng_record) {
		while ((prng_n_gen + _num_byte) > prng_buffer_size)
			prng_expand_buffer();
...
void userrand_bytes(unsigned char* a, unsigned _num_byte) {
	if (user_source) {
		for (unsigned i = 0; i < _num_byte; i++)
			a[i] = _source[(n_generated + i) % user_source];
	} else
		RAND_bytes(a, _num_byte);
