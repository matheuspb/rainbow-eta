...
static inline void
gf256v_fdump(FILE* fp, const uint8_t* v, unsigned _num_byte) {
	fprintf(fp, "[%2d][", _num_byte);
	for (unsigned i = 0; i < _num_byte; i++) {
		fprintf(fp, "0x%02x,", v[i]);
		if (7 == (i % 8))
			fprintf(fp, " ");
	}
	fprintf(fp, "]");
...
static inline void
_gf256v_add(uint8_t* accu_b, const uint8_t* a, unsigned _num_byte) {
	for (unsigned i = 0; i < _num_byte; i++)
		accu_b[i] ^= a[i];
}
...
static inline void
_gf16v_mul_scalar(uint8_t* a, uint8_t gf16_b, unsigned _num_byte) {
	for (unsigned i = 0; i < _num_byte; i++)
		a[i] = gf256_mul_gf16(a[i], gf16_b);
}

static inline void _gf16v_madd(
	uint8_t* accu_c, const uint8_t* a, uint8_t gf16_b, unsigned _num_byte) {
	for (unsigned i = 0; i < _num_byte; i++)
		accu_c[i] ^= gf256_mul_gf16(a[i], gf16_b);
}

static inline void
_gf256v_mul_scalar(uint8_t* a, uint8_t b, unsigned _num_byte) {
	for (unsigned i = 0; i < _num_byte; i++)
		a[i] = gf256_mul(a[i], b);
}

static inline void _gf256v_madd(
	uint8_t* accu_c, const uint8_t* a, uint8_t gf256_b, unsigned _num_byte) {
	for (unsigned i = 0; i < _num_byte; i++)
		accu_c[i] ^= gf256_mul(a[i], gf256_b);
}

static inline void
_gf256v_m0x10_add(uint8_t* accu_c, const uint8_t* a, unsigned _num_byte) {
	for (unsigned i = 0; i < _num_byte; i++)
		accu_c[i] ^= gf256_mul_0x10(a[i]);
}

static inline void
_gf256v_m0x4_add(uint8_t* accu_c, const uint8_t* a, unsigned _num_byte) {
	for (unsigned i = 0; i < _num_byte; i++)
		accu_c[i] ^= gf256_mul_0x4(a[i]);
}

static inline unsigned char gf16v_get_ele(const uint8_t* a, unsigned i) {
	unsigned char r = a[i >> 1];
	r = (i & 1) ? (r >> 4) : (r & 0xf);
	return r;
}

static inline unsigned char gf16v_set_ele(uint8_t* a, unsigned i, uint8_t v) {
...
static inline void gf16v_split(uint8_t* z, const uint8_t* x, unsigned n) {
	for (unsigned i = 0; i < n; i++)
		z[i] = gf16v_get_ele(x, i);
}

static inline unsigned char
_gf16v_dot(const uint8_t* a, const uint8_t* b, unsigned _num_ele) {
	unsigned char r = 0;
...
static inline void gf16v_rand(uint8_t* a, unsigned _num_ele) {
	if (0 == _num_ele)
		return;
...
static inline void gf256v_rand(uint8_t* a, unsigned _num_byte) {
	prng_bytes(a, _num_byte);
}

static inline void
gf16mat_fdump(FILE* fp, const uint8_t* v, unsigned n_vec_byte, unsigned n_vec) {
...
static inline void gf256mat_fdump(
	FILE* fp, const uint8_t* v, unsigned n_vec_byte, unsigned n_vec) {
	gf16mat_fdump(fp, v, n_vec_byte, n_vec);
}

static inline void _gf16mat_prod(
	uint8_t* c, const uint8_t* matA, unsigned n_A_vec_byte, unsigned n_A_width,
	const uint8_t* b) {
...
static inline void
gf16mat_mul(uint8_t* c, const uint8_t* a, const uint8_t* b, unsigned len_vec) {
...
gf16mat_gauss_elim(uint8_t* mat, unsigned h, unsigned w) {
	...
	for (unsigned i = 0; i < h; i++) {
		...
		for (unsigned j = i + 1; j < h; j++) {
			...
			gf16v_madd(
				ai, aj,
				gf16_is_nonzero(gf16v_get_ele(ai, i)) ^
					gf16_is_nonzero(gf16v_get_ele(aj, i)),
				n_w_byte);
		}
...
static inline void gf16mat_submat(
	uint8_t* mat2, unsigned w2, unsigned st, const uint8_t* mat, unsigned w,
	unsigned h) {
...
static inline void gf16mat_subcolumn(
	uint8_t* col, unsigned idx, const uint8_t* mat, unsigned w, unsigned h) {
...
static inline unsigned gf16mat_rand_inv(uint8_t* a, uint8_t* b, unsigned H) {
...
static inline void
gf256v_polymul(uint8_t* c, const uint8_t* a, const uint8_t* b, unsigned _num) {
	for (unsigned i = 0; i < _num * 2 - 1; i++)
		c[i] = 0;
	for (unsigned i = 0; i < _num; i++)
		_gf256v_madd(c + i, a, b[i], _num);
}

static inline void _gf256mat_prod(
	uint8_t* c, const uint8_t* matA, unsigned n_A_vec_byte, unsigned n_A_width,
	const uint8_t* b) {
	gf256v_set_zero(c, n_A_vec_byte);
	for (unsigned i = 0; i < n_A_width; i++) {
...
static inline void
gf256mat_mul(uint8_t* c, const uint8_t* a, const uint8_t* b, unsigned len_vec) {
...
static inline unsigned
gf256mat_gauss_elim(uint8_t* mat, unsigned h, unsigned w) {
	unsigned r8 = 1;
	for (unsigned i = 0; i < h; i++) {
		uint8_t* ai = mat + w * i;
		for (unsigned j = i + 1; j < h; j++) {
			uint8_t* aj = mat + w * j;
			gf256v_madd(
				ai, aj, gf256_is_nonzero(ai[i]) ^ gf256_is_nonzero(aj[i]), w);
		}
...
static inline void gf256mat_submat(
	uint8_t* mat2, unsigned w2, unsigned st, const uint8_t* mat, unsigned w,
	unsigned h) {
	for (unsigned i = 0; i < h; i++) {
...
static inline unsigned gf256mat_rand_inv(uint8_t* a, uint8_t* b, unsigned H) {
