...
static inline void transpose_l1(uint8_t* r, const uint8_t* a) {
	for (unsigned i = 0; i < _O1; i++) {
		for (unsigned j = 0; j < _O1; j++) {
			gf16v_set_ele(
				r + i * _O1_BYTE, j, gf16v_get_ele(a + j * _O1_BYTE, i));
		}
	}
}
...
static inline void transpose_l2(uint8_t* r, const uint8_t* a) {
	for (unsigned i = 0; i < _O2; i++) {
		for (unsigned j = 0; j < _O2; j++) {
			gf16v_set_ele(
				r + i * _O2_BYTE, j, gf16v_get_ele(a + j * _O2_BYTE, i));
		}
	}
}
...
linear_solver_l1(uint8_t* r, const uint8_t* mat_32x32, const uint8_t* cc) {
	uint8_t mat[_O1 * _O1];
	for (unsigned i = 0; i < _O1; i++) {
...
static inline unsigned
linear_solver_l2(uint8_t* r, const uint8_t* mat_32x32, const uint8_t* cc) {
	uint8_t mat[_O2 * _O2];
	for (unsigned i = 0; i < _O2; i++) {
