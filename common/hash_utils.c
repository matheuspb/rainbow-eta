...
static inline int _sha2_str(
	unsigned char* digest, const unsigned char* m, unsigned long long mlen) {
#if 32 == _HASH_LEN
	SHA256_CTX sha256;
	SHA256_Init(&sha256);
...
int sha2_file(unsigned char* digest, FILE* fp) {
	...
	SHA256_CTX sha256;
	SHA256_Init(&sha256);
	while ((nbyte = fread(buffer, 1, _BUFFER_SIZE, fp))) {
		SHA256_Update(&sha256, buffer, nbyte);
		is_read = 0;
	}
	SHA256_Final(digest, &sha256);
#elif 48 == _HASH_LEN
	...
	SHA384_Init(&sha384);
	while ((nbyte = fread(buffer, 1, _BUFFER_SIZE, fp))) {
		SHA384_Update(&sha384, buffer, nbyte);
		is_read = 0;
	}
	SHA384_Final(digest, &sha384);
#elif 64 == _HASH_LEN
	...
	SHA512_Init(&sha512);
	while ((nbyte = fread(buffer, 1, _BUFFER_SIZE, fp))) {
		SHA512_Update(&sha512, buffer, nbyte);
		is_read = 0;
	}
	SHA512_Final(digest, &sha512);
#else
...
static inline int expand_sha2(
	unsigned char* digest, unsigned n_digest, const unsigned char* hash) {
...
int sha2_chain_file(
	unsigned char* digest, unsigned n_digest, const char* f_name) {
...
int sha2_chain_msg(
	unsigned char* digest, unsigned n_digest, const unsigned char* m,
...
int sha2_chain_byte(
	unsigned char* output_bytes, unsigned output_size, unsigned* chain_status,
	unsigned char* chain) {
